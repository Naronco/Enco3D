...
namespace Enco3D
{
	namespace Rendering
	{
		...
		private:
			...
			ILight *m_activeLight{ nullptr };
			Vector3f m_globalAmbientColor;

			Shader *m_textureShader{ nullptr };
			Shader *m_forwardAmbientShader{ nullptr };
		...
		public:
			...
			inline void AddLight(ILight *light) { m_lights.push_back(light); }
			inline void RemoveLight(ILight *light) { m_lights.erase(remove(m_lights.begin(), m_lights.end(), light), m_lights.end()); }
			...
			inline vector<ILight *> GetLights() const { return m_lights; }
			inline Vector3f GetGlobalAmbientColor() const { return m_globalAmbientColor; }
			inline ILight *GetActiveLight() const { return m_activeLight; };
