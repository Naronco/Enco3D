#ifndef __ENCO3D_IWINDOW_H_INCLUDED__
#define __ENCO3D_IWINDOW_H_INCLUDED__
...
namespace enco
{
	namespace core
	{
		namespace input
		{
			const static int numKeys = 512;
			const static int numButtons = 8;
		...
		protected:
			char *m_title;
			Dimension2u32 m_size;
			math::Vector2s32 m_cursorPosition;
			bool *m_keys, *m_buttons;
			bool m_closeRequested, m_focused;

		public:
			virtual void create(char *title, const Dimension2u32 &size, uint8 samples) = 0;
			...
			virtual void setPosition(const math::Vector2s32 &position) = 0;
			virtual void setSize(const Dimension2u32 &size) = 0;
			virtual void setCursorPosition(const math::Vector2s32 &position) = 0;
			virtual void setCursor(bool visible) = 0;

			inline char *getTitle() const { return m_title; }
			inline Dimension2u32 getSize() const { return m_size; }
			inline bool isCloseRequested() const { return m_closeRequested; }
			inline bool isFocused() const { return m_focused; }
			inline math::Vector2s32 getCursorPosition() const { return m_cursorPosition; }
			inline bool isKeyDown(uint16 keyCode) const { return m_keys[keyCode]; }
			inline bool isButtonDown(uint16 buttonCode) const { return m_buttons[buttonCode]; }
		};
	}
}
